...
const departments = computed(() => {
  ...
  emails.value.forEach(email => {
    ...
    if (!map[deptName]) {
      map[deptName] = { id: deptName, name: deptName, total: 0, active: 0, inactive: 0, exited: 0, retired: 0 }
    }
  ...
  <div class="bg-[#0f172a] min-h-screen text-gray-200 p-6">
    ...
    <div class="bg-[#1e293b] rounded-2xl shadow-lg p-6 overflow-x-auto">
      <table class="w-full text-left border-collapse">
        ...
        <tbody>
          <tr v-for="dep in departments" :key="dep.id" class="border-b border-gray-700 hover:bg-[#1a2536] transition-all duration-200">
